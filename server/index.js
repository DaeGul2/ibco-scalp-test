      ...
      messages: [
        {
         ...
         진단이 불가하더라도 일반 텍스트를 보내지말고 반드시 아래의 json형태에따라 보내주세요
          1. 사진을 보고 두피사진에 대해 한국어로 진단을 내려주세요. 단, 사진이 너무 어둡거나 혹은 두피가 너무 멀리 찍혀 있어서 진단이 어렵다면, '진단불가'를 판단하세요.
          2. 진단 가능한 경우, bounding box를 위한 각 박스별 x, y, width, height 값을 {x,y,width,height} object로 배열 안에 담아 보내주세요.
...
` },
    ...
    let aiResponse = response.choices[0].message.content.trim(); // ✅ 응답에서 불필요한 공백 제거
    ...
    const parsedResponse = JSON.parse(aiResponse); // 🔥 JSON 파싱
