      ...
      messages: [
        {
         ...
         진단이 불가하더라도 반드시 일반 텍스트를 보내지말고 반드시 아래의 json형태에따라 보내주세요.
           ...
           **다음 네 가지 평가 항목을 기준으로 점수를 계산하세요.**  
   - "sebum" (유분 상태) → 두피가 번들거리거나 기름기가 있는 경우 지성 두피로 판단 가능  
   - "flaking" (각질 상태) → 비듬, 하얀 각질, 피부 벗겨짐 여부 확인 가능  
   - "moisture" (두피 수분 상태) → 건조한 경우 피부가 거칠거나 각질이 많아짐  
   - "inflammation" (두피 염증 여부) → 붉은 기운, 뾰루지, 붓기 등을 감지 가능  
   - **각 항목별 점수 범위는 0~100점이며, 높을수록 건강한 상태입니다.**
...
- 바디워시, 선크림, 페이셜폼도 비슷한 맥락임
          5. 제품 추천 이유를 두피 상태에 기반하여 제공하세요. 당연히 진단불가면 추천을 안 해야합니다. 또한, 제품의 성분을 잘 파악해서 추측하세요
          6. JSON 형식으로 응답하세요:
            ...
            - scores: {
    "sebum": 점수 (0~100),  // 유분 상태
    "flaking": 점수 (0~100),  // 각질 상태
    "moisture": 점수 (0~100),  // 수분 상태
    "inflammation": 점수 (0~100)  // 염증 여부
  },  
